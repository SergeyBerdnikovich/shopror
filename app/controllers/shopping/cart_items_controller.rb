class Shopping::CartItemsController < Shopping::BaseController
  ...
  def create
    ...
    variant = Variant.includes(:product).find_by_id(params[:variant_id])
    if cart_item = session_cart.add_variant(params[:variant_id], most_likely_user, qty)
      ...
      redirect_to product_path(variant.product.id)
    else
      if variant
